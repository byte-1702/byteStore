...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['game_id'])) {
    ...
    $addedOk = false;
    if ($selected !== null) {
        $stock = (int) ($selected['stock'] ?? 0);
        $inCart = isset($_SESSION['cart'][$gameId]) ? (int) $_SESSION['cart'][$gameId]['quantity'] : 0;
        if ($stock <= 0) {
            $_SESSION['catalog_flash'] = 'Ese producto no tiene stock disponible.';
        } elseif ($inCart + 1 > $stock) {
            $_SESSION['catalog_flash'] = 'No hay unidades suficientes en inventario para agregar mas de este titulo.';
        } elseif (isset($_SESSION['cart'][$gameId])) {
            $_SESSION['cart'][$gameId]['quantity']++;
            $_SESSION['cart'][$gameId]['price'] = (float) $selected['price'];
            $addedOk = true;
        } else {
    ...
    if (!empty($_SESSION['catalog_flash'])) {
        $parts[] = 'notice=1';
    }
    $target = 'productos.php' . (count($parts) > 0 ? '?' . implode('&', $parts) : '');
    header('Location: ' . $target);
    exit;
}

$catalogNotice = '';
if (isset($_GET['notice']) && !empty($_SESSION['catalog_flash'])) {
    $catalogNotice = (string) $_SESSION['catalog_flash'];
    ...
    <div class="home-container products-layout">
        ...
        <div>
            ...
            <?php if ($catalogNotice !== ''): ?>
                <p class="catalog-warning" role="alert"><?php echo htmlspecialchars($catalogNotice, ENT_QUOTES, 'UTF-8'); ?></p>
            <?php endif; ?>
            <div class="products-grid">
                ...
                <?php foreach ($filteredGames as $game): ?>
                    <article class="product-card">
                        ...
                        <p class="product-card__stock">Stock: <?php echo (int) ($game['stock'] ?? 0); ?></p>
                        <form method="post" action="productos.php">
                            ...
                            <button type="submit" class="home-btn home-btn--primary product-card__button" <?php echo ((int) ($game['stock'] ?? 0)) <= 0 ? 'disabled' : ''; ?>>
                                <?php echo ((int) ($game['stock'] ?? 0)) <= 0 ? 'Sin stock' : 'Agregar al carrito'; ?>
                            </button>
                        </form>
                    </article>
                <?php endforeach; ?>
